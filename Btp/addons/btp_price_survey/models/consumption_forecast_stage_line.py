...
class ConsumptionForecastLine(models.Model):
    _name = 'btp_price_survey.consumption_forecast.stage.line'
    ...
    consumption_forecast_stage_id = fields.Many2one('btp_price_survey.consumption_forecast.stage', string='Étape de prévision', ondelete="cascade")
    ...
    def _compute_global_quantity(self):
        consumption_forecast = self.env['btp_price_survey.consumption_forecast'].search(
            [('construction_site_id', '=', self.consumption_forecast_stage_id.consumption_forecast_id.construction_site_id.id),
             ('state', '=', 'close')], limit=1, order='id desc')
        if consumption_forecast:
            last_consumption_stage = False
            for consumption_forecast_stage in consumption_forecast.stage_ids:
                if consumption_forecast_stage.name == self.consumption_forecast_stage_id.name:
                    last_consumption_stage = consumption_forecast_stage
