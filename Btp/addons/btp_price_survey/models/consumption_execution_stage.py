...
class ConsumptionExecutionStage(models.Model):
    _name = 'btp_price_survey.consumption_execution.stage'

    name = fields.Many2one('btp_price_survey.stage', string='Etape', required=True)
    analytical_account = fields.Char(related='name.analytical_account_mane', string='N° compte analytique')
    consumption_execution_id = fields.Many2one('btp_price_survey.consumption_execution',
                                               string="Consommation réelle", ondelete="cascade")
    line_ids = fields.One2many('btp_price_survey.consumption_execution.stage.line', 'consumption_execution_stage_id',
